...
**❌ 错误：** `import module from 'libmodule.so';`
**✅ 正确：** `import { funcA, funcB } from 'libmodule.so';`
...
**⚠️ 不要加 lib 前缀！** CMake 会自动添加 `lib` 前缀。

**❌ 错误：** `add_library(libmp4 SHARED ...)` → 生成 `liblibmp4.so`
**✅ 正确：** `add_library(mp4 SHARED ...)` → 生成 `libmp4.so`

```cmake
cmake_minimum_required(VERSION 3.5.0)
project(mylib)

add_library(mp4 SHARED           # ← 库名不含 lib
    napi_init.cpp
    src/mp4_core.c
)
...
target_include_directories(mp4 PUBLIC
...
| 位置 | 值 | 示例 |
|------|-----|------|
| CMakeLists.txt `add_library()` | 不含 lib | `mp4` |
| 生成文件 | CMake 自动加 lib | `libmp4.so` |
| `oh-package.json5` name | 完整 .so 文件名 | `"libmp4.so"` |
| ArkTS import | 完整 .so 文件名 | `from 'libmp4.so'` |
